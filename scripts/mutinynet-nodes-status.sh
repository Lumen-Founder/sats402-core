...
json_field() {
  local field="$1"
  node -e "const field=process.argv[1];let d='';process.stdin.on('data',c=>d+=c);process.stdin.on('end',()=>{try{const j=JSON.parse(d); console.log(j[field] ?? '')}catch{console.log('')}})" "$field"
}

channel_state() {
  local pubkey="$1"
  node -e "const pubkey=process.argv[1];let d='';process.stdin.on('data',c=>d+=c);process.stdin.on('end',()=>{try{const j=JSON.parse(d); const c=(j.channels || []).find(ch => ch.remote_pubkey === pubkey); if (!c) { console.log('missing'); return; } console.log(c.active ? 'active' : 'inactive')}catch{console.log('unknown')}})" "$pubkey"
}
...
echo "===== containers ====="
docker compose -f docker-compose.nodes.yml ps || true
echo ""
...
if [ "$agent_gateway_state" = "active" ] && [ "$gateway_merchant_state" = "active" ]; then
  ...
  echo "Next: npm run mutinynet:live"
else
  echo "Live demo can run: no"
  echo "Next: npm run nodes:addresses, fund Agent/Gateway at https://faucet.mutinynet.com/, then npm run nodes:connect."
fi
