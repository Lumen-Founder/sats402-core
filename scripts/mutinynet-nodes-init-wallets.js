...
const DEFAULT_PASSWORD = 'sats402-demo-password';
const MAX_WAIT_MS = Number(process.env.SATS402_LND_INIT_TIMEOUT_MS || 180_000);
const roles = [
  { role: 'agent', port: 8081, container: 'sats402-lnd-agent' },
  { role: 'gateway', port: 8082, container: 'sats402-lnd-gateway' },
  { role: 'merchant', port: 8083, container: 'sats402-lnd-merchant' }
];
...
function secretsDir(role) {
  return path.join(process.cwd(), 'secrets', role);
}
...
function loadPassword(role) {
  const envPassword = process.env.SATS402_LND_WALLET_PASSWORD;
...
for (const roleConfig of roles) {
  const { role } = roleConfig;
  process.stdout.write(`[${role}] initializing/unlocking wallet... `);
  try {
...
console.log('\n[SATS-402] LND wallet init summary');
for (const result of results) {
  console.log(`- ${result.role}: ${result.ok ? `OK (${result.status})` : `FAIL (${result.error})`}`);
}

if (results.some((result) => !result.ok)) {
  console.error('\nNext: inspect logs with npm run nodes:logs, then rerun npm run nodes:init.');
  process.exit(1);
}
